...
**主线状态（exp001–012 闭环）**：结构→放电→细胞外场的完整管线已建成
并在功能层验证。2026-09-13 第二轮精度升级（全部带独立验证）：
**① OU 色噪声背景**（ColoredCurrentNoise，τ=8ms σ=60pA → 膜波动
~4mV，替代 0.3mV 的白噪声；LIF f-I 刀锋被磨平，T4 基流不再精确到
个位）；**② 电导基 MT 突触**（数据集符号→翻转电位，g_unit=0.02nS，
E_inh=−80mV；半隐式积分无条件稳定——自然刺激峰值电导下显式欧拉
发散过，已修复并测试）；**③ 全链路传导/突触延迟**（1ms 化学 +
dist/0.3m/s 轴突，每边独立延迟环形缓冲）；**④ 工作点第 3 轮校准**
（scripts/outputs/working_point_calibration_r3.json：黑暗期 MID 22.4
/ T4 5.9 / T5 5.0 Hz，全部文献窗内）；**⑤ 背景人脑 EEG 叠加**
（viz：闭眼 α 波 10Hz 枕区主导 ~30μV + 1/f + 传感器噪声 1.5μV；
页面开关切换叠加；结果：果蝇信号 0.74μV vs 背景 10μV @147°，
**d′=2 需 ~731 次试验平均**——单次不可检出但完全在常规 VEP 实验
范围内）；**⑥ exp012 回流约束实验**（experiments/exp012_return_
constraint/：三几何族暂不可判别——off/on 锚点失败源于缺 L2 反弹
动力学而非几何；neurite 变体波形病理最可疑；绝对幅值 ~10× 杠杆
仍开放，见其 README）。剩余升级：回流判别闭合（加 exp003 Off 动力学
后重跑 exp012 或强度系列锚点）、扩电路到 lobula plate/中央脑、
L1/L2 分级电位神经元、自发背景活动的完整版（真实突触轰击替代
OU）、椭球头几何、髓质逐突触几何。
