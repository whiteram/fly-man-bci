...
**主线状态（exp001–011 闭环）**：结构→放电→细胞外场的完整管线已建成
并在功能层验证（ERG 波形/潜伏期/闪烁融合/方向选择性涌现/多通道场记录/
分层体积导体/递质符号驱动的 ON-OFF 架构）。2026-09-13 精度升级三项完成：
**四球壳前向核**（脑/CSF/颅骨/头皮，FourSpherePairField；σ_CSF=σ_脑时
与三球壳一致到 1e-8，tests/test_foursphere_kernel.py）、**枕极放置**
（网络 T4/T5 端贴颅骨内壁，最强信号移到高夹角电极 141-162°，与真实
视觉诱发电位拓扑一致）、**工作点校准**（scripts/calibrate_working_point.py
两轮扫描：黑暗期 Mi/Tm 40→22.4、T5 134→11.7、T4 0→2.1Hz（文献窗
5-20/2-15/2-12）；T4 需要"未建模背景输入"基流+噪声，因其 Mi1 输入在
黑暗期被 L1 抑制；注意低噪声 LIF 的 f-I 极陡（5↔50Hz 仅差 200→250pA），
工作点对电流敏感）。剩余升级（按价值排序）：回流位置假设的实验约束与
绝对幅值标定（含头壳参数实测）、扩电路到 lobula plate/中央脑、自发背景
活动（真实突触轰击替代白噪声，可治 T4 刀锋问题）、椭球头几何、髓质层
逐突触几何。
