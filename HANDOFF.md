...
**主线状态（exp001–008 闭环）**：结构→放电→细胞外场的完整管线已建成
并在功能层验证（ERG 波形/潜伏期/闪烁融合/方向选择性涌现/多通道场记录/
分层体积导体）。exp008 新增 `ffbm.forward.SealedHeadPairField`（密封
双层球核，极限验证 6e-6）：绝缘头壳使眼通道场放大 ~2.0×、板层 ~1.6×、
髓质 ~1.2×（浅表增益更大），波形与通道结构不变（相关 ≥0.997）——
分层改变标定不改信息；薄/厚壳几乎相同（密封边界主导）。
剩余升级（按价值排序）：回流位置假设的实验约束与绝对幅值标定（含
头壳参数实测）、扩电路到 lobula plate/中央脑、自发背景活动、L2 Off
动力学、椭球头几何。
