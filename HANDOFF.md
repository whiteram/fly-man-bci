...
exp009 工作点的一次 12.2s 闪光串仿真，export_data.py 可再生）：五层真实
坐标点云按放电率脉动、三级连接流、三电极搏动、头壳线框、右侧三通道
电位 + 五层放电率实时图表 + 播放控制。本地服务器打开（python -m
http.server），vendor/ 内置 Three.js r128 离线可用。
