...
分层体积导体/递质符号驱动的 ON-OFF 架构）。剩余升级（按价值排序）：
工作点按真实放电率精调（exp009 遗留）、回流位置假设的实验约束与绝对
幅值标定（含头壳参数实测）、扩电路到 lobula plate/中央脑、自发背景
活动、椭球头几何、髓质层逐突触几何。
...
实时图表 + 播放控制。右上按钮 **🧠 人脑壳视图**把网络 ×400 缩放进脑/颅骨/
头皮三层同心球壳（exp010 几何）：17 个电极均匀贴头皮（0 号在眼轴），
头皮球面按电位插值**实时着色**（topomap，蓝负红正）——exp010/011 思想实验
的可视化。深部源 → 全头近似同步、幅度随夹角平滑衰减，正是真实 EEG 深部源
空间分辨率低的表现。本地服务器打开（python -m http.server），
vendor/ 内置 Three.js r128 离线可用。
