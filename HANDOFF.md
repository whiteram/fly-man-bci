...
**主线状态（exp001–014 闭环，2026-09-14 终版）**：全机制栈已在主管线
运行——机制板层（分级 R/L、组胺氯电导 E_Cl=−70/v_K=−25、L→Mi/Tm 分级
释放，无基流 hack）+ 第 5 轮校准（黑暗 MID 18.3/T4 6.6/T5 1.9 Hz；
L1/2 暗 −35.2mV+深度 20.5mV 双文献窗）+ 回流几何判定 neurite ×1.7 幅值
标定（exp013：文献一致点 off/on 仅 neurite 通过，25 配置 8:0:0）。
终版数字：果蝇信号 0.78μV vs 背景 10.0μV @147°，宽带 d′=2 需 658 次
试验（带限 2780）。校准脚本 scripts/calibrate_r5.py（两阶段，先按
电路突触质量定 g_hist 再定 g_lm——电导与每细胞质量成反比，跨电路
不可直搬）。剩余：exp013 二维参数
