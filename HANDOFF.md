...
> 注：实际执行时 exp003 让位给了"非对称光转导 → Off 瞬变"追问
> （experiments/exp003_asymmetric_off/，2026-09-13 完成），本节运动光栅
> 顺延为 exp004。exp003 结论：正 Off 瞬变需要时间尺度分离（非对称
> 光转导）与突触场符号为正两个条件同时成立；真实 Off 瞬变的发生器
> 大概率是 Off 通路的活动性关断响应（L2 是 Off 细胞）。
