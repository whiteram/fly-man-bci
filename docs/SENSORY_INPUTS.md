...
**MI-v2：DN 驱动通路（已实现，imagery 窗全程可解码）**：DN 输入
探针（`exp020_proprioception/dn_probe.py`）显示 DN 392 万输入突触中
96% 已在仿真电路里（cb_intrinsic 61% = CX/LAL/SEZ 主驱动 + DN→DN
13% + 上升 AN* 13% + VPN 6.7%；PRO 直连仅 0.27%）——v1 延迟窗无信息
缺的是驱动量级而非通路。v2 范式在指令窗后加**想象窗内部驱动**
（1300–3500 ms，无外周输入）：**A 路由方案**驱动同侧 LAL* 群
（317 细胞 1200 pA，经真实 LAL→DN 突触，含 800 ms 适配补偿），
**C 对照**直驱同侧 DN 群（596–712 细胞 800 pA）。24 试次结果：
想象窗类别信息全程显著（A：L−R +0.001~+0.002 µV、p=0.003–0.013；
C：−0.005~−0.008 µV、p=0.001–0.002，DN 谷氨酸能压制故极性反转），
尾窗回落基线、无点火。**"运动想象"状态可在无复反馈下从头皮持续
