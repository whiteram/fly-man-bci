...
| **刺激：本体感觉 + 运动意图解码** | ✅ 首版完成 | 新区域 `proprioception`（PRO 1,454，rootSide L/R 分组；PRO_V→VNC 47.9k 对、PRO_C→中央 4.8k 对）；MI 范式（指令窗 L/R 本体反馈 400 pA → 延迟窗）：指令窗可解码（99–100%，效应 0.3% 背景水平、p=0.002，纯 PRO_C 末梢偶极子）；**延迟窗无保持**（外周/中央严格静默，原带通分析的 78–89% 是 filtfilt 长尾伪影）——真正的"运动想象保持"需招募 DN 层，列为后续 | exp020, docs/SENSORY_INPUTS.md §4 |
