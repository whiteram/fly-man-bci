...
| **BCI 范式：运动想象 / 嗅觉 / 听觉** | ✅ | `motor_imagery`（2 类，复用 exp020 MI-v2）：想象窗空间 LOO 100%（p=0.003）；`olfactory`（3 气味身份，exp019 电路，ORN 群大小补偿幅度）：3 类 89%（p=0.0005，DA1/DM2/VA1v 空间模式可分）；`auditory`（新区域 `audition`：JO-A 115→JOA_C 1,578 对/24,392 突触→中央脑；求偶歌包络建模）：4 类 54%（p=0.007）/ 侧别 75% / 模式 75%——类间总 rms 相同，可分性全在空间模式 | bci/*, exp021, docs/SENSORY_INPUTS.md §4-5 |
