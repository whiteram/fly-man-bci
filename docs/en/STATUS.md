...
| **Stimuli: proprioception + motor-intention decoding** | ✅ first pass | new region `proprioception` (PRO 1,454, rootSide L/R groups; PRO_V→VNC 47.9k pairs, PRO_C→central 4.8k); MI paradigm (instruction-window L/R reafference @400 pA → delay window): cue decodable (99–100%, effect 0.3% of background, p=0.002, pure PRO_C terminal dipole); **no delay-window persistence** (periphery and central strictly silent; the 78–89% from the first band-envelope analysis was a filtfilt long-tail artifact) -- true imagery holding needs DN-layer recruitment, listed as follow-up | exp020, docs/SENSORY_INPUTS.md §4 |
