...
def main():
    ...
    print("== state decode (dark trials, quiet vs active) ==")
    X = np.array([np.sqrt((p[WIN[0]:WIN[1]] ** 2).mean(axis=0))
                  for state in meta["states"]
                  for p in phis[(state, "dark")]])
    y = np.array([0] * len(phis[("quiet", "dark")])
                 + [1] * len(phis[("active", "dark")]))
    acc = loo_acc(X, y)
    ...
    print(f"dark floor rms: quiet {floors['quiet']:.4f} vs active "
          f"{floors['active']:.4f} uV | state LOO {acc:.0%} "
          f"perm-p={p:.4f}")
