"""bci/dangate2 analysis: circuit-level error coding + emergent blocking.
...
def main(prefix=""):
    ...
    print(f"[dangate2] {'PILOT ' if prefix else ''}"
    ...
    if "blocked" in res and "control" in res and not prefix:
        ...
        print(f"\n[dangate2] w_B final: blocked {np.mean(fb):.4f} vs "
              f"control {np.mean(fc):.4f}  -> ratio {ratio:.2f}x "
              f"(>1 = blocking emerged)")
        print(f"[dangate2] mod_ms p2 mean: blocked {mb:.1f} vs control "
    ...
    (OUT / f"{prefix}summary.json").write_text(json.dumps(
        ...
        indent=1))
    print(f"[dangate2] summary -> {OUT / f'{prefix}summary.json'}")
