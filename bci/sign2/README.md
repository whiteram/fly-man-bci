...
默认 CEN_C 增益=0.004——mmdev2 气味协议的实际工作点；0.5× 的
条件化工作点下 base 的 AL 响应本来就只有 1/50，见 halfbase 臂）。
...
3. **0.5× 工作点不支撑气味协议**：halfbase 的 ALPN 只有 +0.22 Hz
   （默认增益的 1/50）——气味协议活在默认增益（mmdev2 口径）。
