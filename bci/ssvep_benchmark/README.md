...
- **混淆矩阵**（`outputs/confusion_matrix_{clean,noise}.png`，另存
  `rho_matrix_*.npy`）：左图为离散预测计数（对角=正确，红叉=错误），
  右图为每个（真实试次 × 候选频率）的 CCA ρ 矩阵。可见结构：
  **9.8 Hz 是"吸引子"**（5 个错误都预测成它，ρ 列整体偏亮）——该
  试次响应过强产生泛化；13.4 Hz 试次的 ρ 行整体偏亮（宽带强响应，
  与所有候选都相关，但对角仍最高）；其余错误多为相邻 0.2 Hz 类
