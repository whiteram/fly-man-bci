...
| **眼睛** | 刺激可以是真实的人类视频：灰度 + 小网膜点扩散模糊 → 分位数归一化 → 按每只光感受器的眼平面坐标仿射采样 → 逐受体亮度进入级联（合成 1/f 闪烁与漂移纹理协议也内置）。 |
| **头** | 几何：**Lee Perry-Smith 三维头部扫描**（CC-BY 3.0，经 three.js 素材分发）。物理：经典四球壳模型（脑 / 脑脊液 / 颅骨 / 头皮；文献半径与电导率）——每份突触电流是一个源-汇偶极，60 阶勒让德展开把场传到头皮。 |
| **脑电** | 可切换的国际 10-20 系电极帽：**45 导（10-20）· 64 导（10-10）· 128 导（10-5）· EGI HydroCel 256**（241 个可用位点），1 kHz 采样；可叠加逼真背景脑电（α 节律、1/f 活动、传感器噪声）；SNR 与 d′ 检测分析。 |
...
- **GPU 加速** —— CuPy 路线加速前向核与仿真循环（规划中，路线图见 `docs/ACCELERATION_PLAN.md`）
- **闭环** —— 解码输出回馈到刺激

## 引擎近况

勒让德核构建多线程化（**约 15× 提速，123 万偶极对 68 s**），缓冲式 float32 前向
路径与 float64 基线对拍误差为峰值的 0.092%；全 CNS 导出在 16 核台式机上端到端
可跑。要求：Python 3.12，NumPy / SciPy / pandas / pyarrow（电极与视频工具另需
MNE-Python 和 imageio/PyAV）；连接组下载约 14 GB（公开直链、无需注册）；
完整管线建议 16 GB 以上内存。
